...
class ControllerSaleCustomer extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($this->request->get['filter_external_id'])) {
				$url .= '&filter_external_id=' . $this->request->get['filter_external_id'];
			}
	...
	protected function getForm() {
		...
		$data['customer_groups'] = $this->model_sale_customer_group->getCustomerGroups();
		$data['contact_types'] = $this->model_sale_customer->getContactTypes();
		...
		if (isset($this->request->post['external_id'])) {
			$data['external_id'] = $this->request->post['external_id'];
		} elseif (!empty($customer_info)) {
			$data['external_id'] = $customer_info['external_id'];
		} else {
			$data['external_id'] = $this->config->get('external_id');
		}
        ...
        if (isset($this->request->post['comment_customer'])) {
            $data['comment_customer'] = $this->request->post['comment_customer'];
        } elseif (!empty($customer_info)) {
            $data['comment_customer'] = $customer_info['comment_customer'];
        } else {
		...
		if (isset($this->request->post['password'])) {
			$data['password'] = $this->request->post['password'];
                        ...
                        $data['password'] = substr(md5(time()), 0, 6);
		} else {
			$data['password'] = '';
		}
		...
		if (isset($this->request->post['address'])) {
			...
			$data['address'] = $this->model_sale_customer->getAddressByCustomerId($this->request->get['customer_id']);
		} else {
			$data['address'] = array(
                'zone_id' => 0,
                'city' =>'',
		...
		if (isset($this->request->post['customer_contacts'])) {
			$data['customer_contacts'] = $this->request->post['customer_contacts'];
		} elseif (isset($this->request->get['customer_id'])) {
			$data['customer_contacts'] = $this->model_sale_customer->getContacts($this->request->get['customer_id']);
		} else {
	...
	protected function validateForm() {
		...
		if (!isset($this->request->get['customer_id'])) {
			...
			if ($customer_info && ($this->request->get['customer_id'] != $customer_info['customer_id'])) {
                ...
                $t_length = utf8_strlen($this->format->formatNumbers($this->request->post['telephone'], true));
		if ($t_length != 12) {
		...
		if (isset($this->request->post['address'])) {
            $value = $this->request->post['address'];
				...
				if ((utf8_strlen($value['address_1']) < 3) || (utf8_strlen($value['address_1']) > 128)) {
					$this->error['address']['address_1'] = $this->language->get('error_address_1');
				}

				if ((utf8_strlen($value['city']) < 2) || (utf8_strlen($value['city']) > 128)) {
					$this->error['address']['city'] = $this->language->get('error_city');
				}
