...
class ControllerUserUserPermission extends Controller {
	...
	protected function validateDelete() {
        ...
        public function access() {
                $json = array('success' => true, 'groups' => array());
                $route = isset($this->request->post['_route']) ? $this->request->post['_route'] : '';
                $els = isset($this->request->post['els']) ? $this->request->post['els'] : array();
                        
                $accesses = $this->user->getAccesses($route, $els);
                foreach($accesses as $access) {
                        if(!isset($json['groups'][$access['user_group_id']])) {
                                $json['groups'][$access['user_group_id']] = array(
                                        'user_group_id' => $access['user_group_id'],
                                        'access'        => array()
                                );
                        }
                        $json['groups'][$access['user_group_id']]['access'][] = $access['value'];
                }
                        
                $accesses = $this->user->getAccesses('left-menu', $els);
                foreach($accesses as $access) {
                        if(!isset($json['groups'][$access['user_group_id']])) {
                                $json['groups'][$access['user_group_id']] = array(
                                        'user_group_id' => $access['user_group_id'],
                ...
                $accesses = $this->user->getAccesses('top-menu', $els);
                foreach($accesses as $access) {
                        if(!isset($json['groups'][$access['user_group_id']])) {
                                $json['groups'][$access['user_group_id']] = array(
                                        'user_group_id' => $access['user_group_id'],
        ...
        public function update() {
                ...
                $json = array();
                $this->request->post['name'] = 'dddddd';
                
                if($this->validateForm()) {
                    
                        $route = isset($this->request->post['_route']) ? $this->request->post['_route'] : '';
                        $els = isset($this->request->post['els']) ? $this->request->post['els'] : array();
                        $access = isset($this->request->post['data']) ? $this->request->post['data'] : array();

                        $this->user->deleteAccess(array('route' => $route, 'els' => $els));
                        $this->user->deleteAccess(array('route' => 'left-menu', 'els' => $els));
                        $this->user->deleteAccess(array('route' => 'top-menu', 'els' => $els));
                        foreach($access as $value) {
                                if(substr($value['value'], 0, 3) == 'lm-') {
                                        $this->user->addAccess(array('route' => 'left-menu', 'value' => $value['value'], 'user_group_id' => $value['name']));
                                } elseif(substr($value['value'], 0, 3) == 'tm-') {
                                        $this->user->addAccess(array('route' => 'top-menu', 'value' => $value['value'], 'user_group_id' => $value['name']));
                                } else {
                                        $this->user->addAccess(array('route' => $route, 'value' => $value['value'], 'user_group_id' => $value['name']));
                                }
                        }
                        $json['success'] = true;
                } else {
