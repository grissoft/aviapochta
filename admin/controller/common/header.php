	...
	public function index() {
		$data['title'] = $this->document->getTitle();
			...
			$data['base'] = HTTP_SERVER;
                ...
                if($this->user->getGroupId() == 1) {
                        $this->document->addScript('view/javascript/admin.js');
                }
		...
		if (!isset($this->request->get['token']) || !isset($this->session->data['token']) && ($this->request->get['token'] != $this->session->data['token'])) {
			...
			$data['home'] = $this->url->link('common/dashboard', '', 'SSL');
                        ...
                        $data['access'] = $this->user->getAccess();
                        $data['access'] = $data['access'] ? json_encode($data['access']) : 'false';
...
//                                }
            ...
            $data['sandbox'] = $this->user->getSandbox();
            $data['sandbox_on'] =  $this->url->link($this->request->get['route'], 'sandbox=1&token=' . $this->session->data['token'], 'SSL');
            $data['sandbox_off'] =  $this->url->link($this->request->get['route'], 'sandbox=0&token=' . $this->session->data['token'], 'SSL');
